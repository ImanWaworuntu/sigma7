        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6 flex flex-col items-center text-center relative print:shadow-none print:border-slate-300">
          ...
          <div className="w-full bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-xl p-5 mb-6 border border-slate-200/60 relative overflow-hidden shadow-inner">
            ...
            <div className="flex flex-col gap-3 text-left relative z-10 w-full">
                <div className="flex flex-col gap-2 bg-white p-3 md:p-4 rounded-xl border border-slate-100 shadow-sm transition-all hover:shadow-md hover:border-slate-200 group w-full">
                    ...
                    <strong className="text-slate-800 text-sm md:text-base font-bold text-left break-words pl-[52px]">{student.homeroomTeacher || '-'}</strong>
                </div>
                ...
                <div className="flex flex-col gap-2 bg-white p-3 md:p-4 rounded-xl border border-slate-100 shadow-sm transition-all hover:shadow-md hover:border-slate-200 group w-full">
                    ...
                    <strong className="text-slate-800 text-sm md:text-base font-bold text-left break-words pl-[52px]">{student.phone || '-'}</strong>
                </div>
                ...
                <div className="flex flex-col gap-2 bg-white p-3 md:p-4 rounded-xl border border-slate-100 shadow-sm transition-all hover:shadow-md hover:border-slate-200 group w-full">
                    ...
                    <strong className="text-slate-800 text-sm md:text-base font-bold text-left break-words pl-[52px]">{student.parentPhone || '-'}</strong>
                </div>
                ...
                <div className="flex flex-col gap-2 bg-white p-3 md:p-4 rounded-xl border border-slate-100 shadow-sm transition-all hover:shadow-md hover:border-slate-200 group w-full">
                    ...
                    <strong className="text-slate-800 text-sm md:text-base font-bold text-left break-words pl-[52px]">{student.address || '-'}</strong>
                </div>
            </div>
          </div>
              ...
              history.map((record) => (
                <div key={record.id} className="p-4 border-b border-slate-50 last:border-0 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-800 leading-tight mb-1.5">{record.description || record.desc}</p>
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                      <p className="text-xs font-semibold text-slate-500 whitespace-nowrap">{record.date}</p>
                      <span className="text-[10px] text-slate-400 border-l border-slate-300 pl-2 whitespace-nowrap">Oleh: {record.reportedBy || 'Sistem'}</span>
                      {(record.hasPhoto || record.photoUrl) ? (
                        <button 
                          onClick={() => setPhotoViewUrl(record.photoUrl || `/bukti?id=${record.id}`)} 
                          className="text-[10px] font-bold bg-blue-50 text-blue-600 px-2 py-1 rounded border border-blue-200 hover:bg-blue-100 transition-colors flex items-center gap-1 print:hidden cursor-pointer whitespace-nowrap"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                          Lihat Bukti
                        </button>
                      ) : (
                        <span className="text-[10px] text-slate-400 border-l border-slate-300 pl-2 whitespace-nowrap print:hidden">Tidak ada foto</span>
                      )}
